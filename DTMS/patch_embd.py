...
class PatchEmbed_chan(nn.Module):
    """ 2D Image to Patch Embedding
    """
    def __init__(self, img_size=13, patch_size=1, in_chans=64, embed_dim=512, center_size=7, norm_layer=None, flatten=True):
        super().__init__()
        img_size = to_2tuple(img_size)
        patch_size = to_2tuple(patch_size)
        self.img_size = img_size
...
class PatchEmbed_center(nn.Module):
    """ 2D Image to Patch Embedding
    """
    def __init__(self, img_size=7, patch_size=1, in_chans=64, embed_dim=512, center_size=7, norm_layer=None, flatten=True):
        super().__init__()
        img_size = to_2tuple(img_size)
        patch_size = to_2tuple(patch_size)
        self.img_size = img_size
        ...
        self.proj = nn.Linear(self.patch_size^2, embed_dim)
        self.norm = norm_layer(embed_dim) if norm_layer else nn.Identity()
...
class PositionEmbed(nn.Module):
    def __init__(self, num_patches=64, d_model=768, num_tokens=0):
        super().__init__()
