...
from patch_embd import PatchEmbed_spa,PatchEmbed_chan,PositionEmbed
from mlp import Mlp,Mlp_wo_gate
from drop import DropPath
from weight_init import trunc_normal_
from mask_embeeding import MaskEmbeeding,UnMaskEmbeeding_chan,UnMaskEmbeeding_center
...
class VisionTransformer(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=64, num_classes=1000, embed_dim=768, depth=5,
                 num_heads=12, mlp_ratio=4., qkv_bias=True, drop_rate=0., attn_drop_rate=0., drop_path_rate=0., 
                 center_size=7,embed_layer_chan=PatchEmbed_chan, pos_embed="cosine",
        ...
        if pos_embed == "cosine":
            ...
            self.pos_embed_chan = PositionEmbed(64, embed_dim, self.num_tokens)().to(self.device)
            ...
            self.pos_embed_center = nn.Parameter(torch.zeros(1, num_patches_chan + self.num_tokens, embed_dim)).to(self.device)
        ...
        if self.vit_type == "decoder":
            ...
            self.unmask_embed_center = UnMaskEmbeeding_center(img_size,
                                                        embed_dim,
                                                        in_chans,
                                                        patch_size,
                                                        num_patches_chan,args
    ...
    def autoencoder(self, x):
        ...
        return norm_embeeding_chan, sample_index_chan, mask_index_chan
    
    
    def decoder(self, x_chan, sample_index_chan, mask_index_chan):
        ...
        return decoder_embeeding_chan

    def decodercenter(self, x_center, sample_index_center, mask_index_center):
        """decoder the all patch embeeding with the mask and position embeeding
        """
        # unmask the patch embeeidng with the encoder embeeding
        # print(x_spa.device)

        decoder_embed_center = self.unmask_embed_center(x_center, sample_index_center, mask_index_center)


        x_center = decoder_embed_center + self.pos_embed_chan
        # decoder
        decoder_embeeding_center = self.blocks_chan(x_center)
        return decoder_embeeding_center
    ...
    def forward_features(self, x):
        ...
        x_chan = self.patch_embed_chan(x)   #(32,64,320)
        # add cls token for classification

        cls_token_chan = self.cls_token_chan.expand(x.shape[0], -1, -1)  #(32,1,320)

        x_chan = torch.cat((cls_token_chan, x_chan), dim=1)     #(32,65,320)


        x_chan = self.pos_drop(x_chan + self.pos_embed_chan)    #(32,65,320)

        x_chan = self.blocks_chan(x_chan)   #(32,65,320)

        x_chan = self.norm(x_chan)  #(32,65,320)
        return x_chan
    ...
    def forward(self, x):
        x_chan = self.forward_features(x)   #(32,65,320)
