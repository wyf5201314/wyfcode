...
def list_to_colormap(x_list):
    ...
    for index, item in enumerate(x_list):
        ...
        if item == 10:
            y[index] = np.array([255, 255, 125]) / 255.       #255, 255, 125
        if item == 11:
        ...
        if item == 20:
            y[index] = np.array([229, 204, 255]) / 255.
        if item == 21:
            y[index] = np.array([153, 255, 255]) / 255.
        if item == 22:
...
def get_cls_map(net, device, all_data_loader, y, dataset='IP', encoder_num = 1):
    ...
    classification_map(y_re, y, 300,
                       'results/classification_maps/' + dataset + str(encoder_num) +'_predictions.png')
    classification_map(gt_re, y, 300,
                       'results/classification_maps/' + dataset + '_gt.png')
